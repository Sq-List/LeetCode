 ...
 * [468]验证IP地址
 * @author SqList
 * @createTime 2022-05-30 15:37:02
 **/
public class Q468ValidateIpAddress {
    public static void main(String[] args) throws IOException {
        String file = Objects.requireNonNull(Q468ValidateIpAddress.class.getResource("/testcase/Q468")).getFile();
        BufferedReader bufferedReader = new BufferedReader(new FileReader(file));
        String line;
        while ((line = bufferedReader.readLine()) != null) {
            Solution solution = new Q468ValidateIpAddress().new Solution();

            String out = line + ": " + solution.validIPAddress(line);
    ...
    class Solution {
        public String validIPAddress(String queryIP) {
            return (queryIP + ".").matches("(((\\d)|([1-9]\\d)|(1\\d{2})|(2[0-4]\\d)|(25[0-5]))\\.){4}$") ? "IPv4" :
                    (queryIP + ":").matches("(([\\da-fA-F]{1,4}):){8}$") ? "IPv6" : "Neither";
        }
    }
